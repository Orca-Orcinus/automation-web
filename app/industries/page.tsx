  ...
  return (
    <div className="container mx-auto px-4 py-24">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {[
          { name: "Automotive", desc: "Assembly lines, welding cells, AGVs." },
          { name: "Food & Beverage", desc: "Hygienic washdown systems, packaging." },
          { name: "Medical Device", desc: "Cleanroom automation, assembly." },
          { name: "Electronics", desc: "PCB handling, SMT processes." },
          { name: "Pharma", desc: "Precision dosing, track & trace." },
          { name: "Logistics", desc: "Warehouse sorting, palletizing." },
        ].map((ind) => (
          <div key={ind.name} className="p-8 bg-slate-900 rounded-x1 hover:bg-slate-500 transition-colors">
            <h2 className="text-2xl font-bold mb-5">{ind.name}</h2>
            <p className="text-slate-300">{ind.desc}</p>
          </div>
        ))}
      </div>
